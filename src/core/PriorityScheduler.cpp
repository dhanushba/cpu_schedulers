...
void PriorityScheduler::tick() {
      ...
      for (int i = 0; i < processes.size(); ++i) {
        if (Topprocess.pid == processes[i].pid) {
          currentRunningProcessIndex =
              i; // it actually holds the process that's working on cpu to hold
  ...
  ///////////if NOT IDLE
    ...
    currentPID = processes[currentRunningProcessIndex]
                     .pid; // GET THE process PID current working
    processes[currentRunningProcessIndex]
        .remainingTime--; // DECREMENT PROCESS WORKING RIGHT NOW
    ...
    // check it's preemptive and queue not empty
      ...
      if (priorityQueue.top().priority <
          processes[currentRunningProcessIndex].priority) {
        // the process exit in the queue higher in priority than the current
        // running process add the current process in the queue
        priorityQueue.push(
            processes[currentRunningProcessIndex]); // add the process in the
                                                    // queue
        Process new_process =
            priorityQueue.top(); // the process that need to be executed
        ...
        priorityQueue
            .pop(); // remove it it works now on cpu we don't need it in QUEUE
      }
    }
  }
  ...
  if (!ganttChart.empty() && ganttChart.back().pid == currentPID) {
    ganttChart.back().endTime =
        currentTime + 1; // stretching the interval of this process extra time
  } else {
    // no this is NEW PROCESS ADD IT
    ganttChart.push_back(
        ExecutionRecord(currentPID, currentTime, currentTime + 1));
  }
...
// cout processes
vector<Process> PriorityScheduler::getProcesses() const { return processes; }
// cout ganttchart
    ...
    for (int i = 0; i < processes.size(); ++i) {
      ...
      if (processes[i].pid == pid) {
        if (currentRunningProcessIndex == i) {
          currentRunningProcessIndex = -1;
        } // return it to IDLE AGAIN
        else if (currentRunningProcessIndex > i) {
          // because if we have running process at 7 and we remove one from 3 ,7
          // need to be 6
          // what if the current running process is at 3 and the removed from 5
          // no decrement is needed
          currentRunningProcessIndex--; // decrement the index since we will
                                        // remove one process from original
                                        // vector
        }
